#!/usr/bin/env bash
# Rust binary + OpenSpec template: flake check + content tests.
# Run from repo root: ./tests/rust-bin-openspec/test.sh
# Optional: SKIP_CONTENT_TESTS=1 to only run flake check (faster).
set -euo pipefail
REPO_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
cd "$REPO_ROOT"

echo "==> Rust-bin-OpenSpec template: flake check"
nix flake check "path:$REPO_ROOT/templates/rust-bin-openspec" "$@"
...
echo "==> Rust-bin-OpenSpec template: content tests (nix flake new -t + devShell + build)"
TMPDIR="${TMPDIR:-/tmp}"
TEST_DIR=$(mktemp -d "${TMPDIR}/rust-bin-openspec-template-test.XXXXXXXX")
trap 'rm -rf "$TEST_DIR"' EXIT

nix flake new "$TEST_DIR" -t "path:$REPO_ROOT#rust-bin-openspec"
...
echo "==> Rust-bin-OpenSpec template: all checks passed"
