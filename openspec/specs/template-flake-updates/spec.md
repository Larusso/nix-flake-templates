...
#### Scenario: Updates are proposed via PR; merge by GitHub settings

- **WHEN** the update workflow has changes and tests pass
- **THEN** it opens a pull request with the updated lock files; merge is handled by GitHub settings (e.g. auto-merge when required checks pass), not by the workflow itself
