...
- Add an independent helper flake under `flakes/oh-my-cloudecode/` that packages the upstream project at `https://github.com/Yeachan-Heo/oh-my-claudecode`, renaming its public-facing identifiers from `claudecode` to `cloudecode`.
...
- `oh-my-cloudecode-flake`: Independent helper flake for the upstream `oh-my-claudecode` ecosystem, hosted in this repo under the renamed public identity `oh-my-cloudecode`.
...
- **Repository layout:** a new `flakes/` subtree is introduced for small non-template flakes, including `flakes/oh-my-cloudecode/` and `flakes/oh-my-codex/`.
- **Packaging work:** the repo gains two hosted helper flakes that package upstream projects directly: `oh-my-cloudecode` for `Yeachan-Heo/oh-my-claudecode` and `oh-my-codex` for `Yeachan-Heo/oh-my-codex`.
