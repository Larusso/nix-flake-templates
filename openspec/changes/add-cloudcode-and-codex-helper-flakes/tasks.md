...
- [x] 2.1 Rename the hosted `oh-my-claudecode` helper-flake identity to `oh-my-cloudecode` in public-facing descriptions, docs, and output names where this repo defines them
...
- [x] 2.5 Ensure `flakes/oh-my-cloudecode/` exposes both a default package and a files-only package

## 3. Root Flake Integration

- [x] 3.1 Re-export the default and files-only packages from `flakes/oh-my-cloudecode/` in the root flake
...
- [x] 4.4 Document the hosted helper flakes `oh-my-cloudecode` and `oh-my-codex` in the root README
- [x] 4.5 Write `flakes/oh-my-codex/README.md` in a structure similar to the existing cloudecode packaging README, adapted for `oh-my-codex`
...
- [x] 5.3 Verify that `flakes/oh-my-cloudecode/` and `flakes/oh-my-codex/` are each directly evaluable as standalone flakes
...
- [x] 6.1 Add `tests/oh-my-cloudecode/test.sh` for direct helper-flake builds and root package re-exports
